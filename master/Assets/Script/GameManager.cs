...
public class GameManager : MonoBehaviour {
	...
	public int activeBalon= 1;// balon =2, 99
	...
	private int[,] _boardPieces = new int[Ancho,Alto];
	void OnGUI()
	{
	...
	{
		for (int i = 3; i < 8; i++) {
			Object.Instantiate(CubeDark, new Vector3(0, _boardHeight, i), Quaternion.identity);
			
			Object.Instantiate(CubeDark, new Vector3(14, _boardHeight, i), Quaternion.identity);
	...
	void AddPieces()
	{
		...
		CreatePiece("ficha", 3, 1, 1); // Create all white pawn
		CreatePiece("ficha", 3, 6, -1); // Create all white pawn
		CreatePiece("Rook", 7, 5, 0);
	...
	void CreatePiece(string _pieceName, int _posX, int _posY, int _playerTag)
	{
		...
		else if(_playerTag == -1)
		{
			...
			//_PieceToCreate.tag = "2";
			_PieceToCreate.GetComponent<Renderer>().material = Balon;		
	...
	// aca es cuando se le aprieta encima y cambia de color
	...
	public void MovePiece(Vector2 _coordToMove)
	{
		...
		// se checkea que el movimiento sea valido o sino no realiza
	...
	bool TestMovement(GameObject _SelectedPiece, Vector2 _coordToMove)
	{
		...
		{
		case "ficha": // puede realizar hasta 2 movimientos a la redonda
			if(Mathf.Abs(_deltaX + _deltaY) < 3) 
			{
				_movementLegalBool = true;
			}
			break;
		case "Rook":// puede realizar hasta 4 movimientos a la redonda
			if(Mathf.Abs(_deltaX + _deltaY) < 5) 
			{
				_movementLegalBool = true;
			}
			break;
...
/*public void EatPiece(GameObject _PieceToEat)
	{
		...
		if(TestMovement (SelectedPiece, _coordToEat) && (SelectedPiece.name != "ficha" ||  _deltaX != 0))
		{
